...
CloseBtn.Parent = self.Main:WaitForChild("TopBar")
...
CloseBtn.MouseButton1Click:Connect(function()
	...
	function Lib:CreateTab(name, icon) -- Added 'icon' parameter
    local Library = self 
    ...
    if icon then
        if tabBtn:FindFirstChild("ImageLabel") then
            tabBtn.ImageLabel.Image = "rbxassetid://" .. tostring(icon:gsub("rbxassetid://", ""))
        end
    else
        tabBtn.ImageLabel.Visible = false -- Hide if no icon is provided
        tabBtn.UIPadding.PaddingLeft = UDim.new(0, 10) -- Adjust padding if no icon
    end
		...
		function TabMethods:CreateButton(text, callback, icon) -- Added 'icon' parameter
    ...
    if icon then
        ...
        btnIcon.Position = UDim2.new(1, -10, 0.5, 0) -- Positioned on the right
        btnIcon.Size = UDim2.new(0, 16, 0, 16)
        btnIcon.Image = "rbxassetid://" .. tostring(icon:gsub("rbxassetid://", ""))
